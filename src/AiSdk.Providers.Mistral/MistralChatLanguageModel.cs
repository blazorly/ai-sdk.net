...
/// <summary>
/// Mistral AI implementation of ILanguageModel.
/// </summary>
public class MistralChatLanguageModel : ILanguageModel
{
    ...
    {
        // Mistral AI supports image URLs natively for vision-capable models
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MistralChatLanguageModel"/> class.
    /// </summary>
    ...
    public MistralChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<MistralResponse>(cancellationToken)
            ?? throw new MistralException("Failed to deserialize Mistral AI response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<MistralStreamResponse>(data);
    ...
    private MistralRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new MistralMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new MistralTool
                {
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(MistralResponse response)
    {
    ...
    private static Usage MapUsage(MistralUsage usage) => new Usage(
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                ...
                throw new MistralException(
                    errorResponse?.Error.Message ?? "Mistral AI API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new MistralException($"Mistral AI API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
