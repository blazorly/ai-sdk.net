...
using AiSdk.Providers.Lepton.Models;

namespace AiSdk.Providers.Lepton;

/// <summary>
/// Lepton AI implementation of ILanguageModel.
/// </summary>
public class LeptonChatLanguageModel : ILanguageModel
{
    private readonly HttpClient _httpClient;
    private readonly LeptonConfiguration _config;
    ...
    {
        // Lepton AI currently does not support image URLs
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="LeptonChatLanguageModel"/> class.
    /// </summary>
    ...
    public LeptonChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<LeptonResponse>(cancellationToken)
            ?? throw new LeptonException("Failed to deserialize Lepton AI response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<LeptonStreamResponse>(data);
    ...
    private LeptonRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new LeptonMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new LeptonTool
                {
                    Function = new LeptonFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(LeptonResponse response)
    {
    ...
    private static Usage MapUsage(LeptonUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<LeptonErrorResponse>(content);
                throw new LeptonException(
                    errorResponse?.Error.Message ?? "Lepton AI API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new LeptonException($"Lepton AI API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
