...
using AiSdk.Providers.LlamaFile.Models;

namespace AiSdk.Providers.LlamaFile;

/// <summary>
/// LlamaFile implementation of ILanguageModel.
/// Provides local LLM execution using llamafile's OpenAI-compatible API.
/// </summary>
public class LlamaFileChatLanguageModel : ILanguageModel
{
    private readonly HttpClient _httpClient;
    private readonly LlamaFileConfiguration _config;
    ...
    /// </summary>
    public string Provider => "llamafile";
    ...
    {
        // LlamaFile currently does not support image URLs in local mode
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="LlamaFileChatLanguageModel"/> class.
    /// </summary>
    /// <param name="modelId">The model ID (e.g., "llama-3", "mistral-7b-instruct", or "local-model").</param>
    /// <param name="config">The LlamaFile configuration.</param>
    /// <param name="httpClient">Optional HTTP client to use.</param>
    public LlamaFileChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<LlamaFileResponse>(cancellationToken)
            ?? throw new LlamaFileException("Failed to deserialize LlamaFile response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<LlamaFileStreamResponse>(data);
    ...
    private void ConfigureHttpClient()
    {
        ...
        // Only set Authorization header if API key is provided and not empty
    ...
    private LlamaFileRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new LlamaFileMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        var request = new LlamaFileRequest
        {
            Model = _modelId,
            Messages = messages,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new LlamaFileTool
                {
                    Function = new LlamaFileFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(LlamaFileResponse response)
    {
    ...
    private static Usage MapUsage(LlamaFileUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<LlamaFileErrorResponse>(content);
                throw new LlamaFileException(
                    errorResponse?.Error.Message ?? "LlamaFile API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new LlamaFileException($"LlamaFile API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
