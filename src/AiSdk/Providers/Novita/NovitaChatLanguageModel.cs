...
using AiSdk.Providers.Novita.Models;

namespace AiSdk.Providers.Novita;

/// <summary>
/// Novita implementation of ILanguageModel.
/// </summary>
public class NovitaChatLanguageModel : ILanguageModel
{
    ...
    {
        // Novita currently does not support image URLs
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="NovitaChatLanguageModel"/> class.
    /// </summary>
    ...
    public NovitaChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<NovitaResponse>(cancellationToken)
            ?? throw new NovitaException("Failed to deserialize Novita response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<NovitaStreamResponse>(data);
    ...
    private NovitaRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new NovitaMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new NovitaTool
                {
                    Function = new NovitaFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(NovitaResponse response)
    {
    ...
    private static Usage MapUsage(NovitaUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<NovitaErrorResponse>(content);
                throw new NovitaException(
                    errorResponse?.Error.Message ?? "Novita API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new NovitaException($"Novita API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
