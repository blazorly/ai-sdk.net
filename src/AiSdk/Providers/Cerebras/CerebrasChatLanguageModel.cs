...
using AiSdk.Providers.Cerebras.Models;

namespace AiSdk.Providers.Cerebras;

/// <summary>
/// Cerebras implementation of ILanguageModel.
/// Cerebras provides ultra-fast inference powered by the world's largest AI processor.
/// </summary>
public class CerebrasChatLanguageModel : ILanguageModel
{
    ...
    {
        // Cerebras currently does not support image URLs like OpenAI
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="CerebrasChatLanguageModel"/> class.
    /// </summary>
    ...
    public CerebrasChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<CerebrasResponse>(cancellationToken)
            ?? throw new CerebrasException("Failed to deserialize Cerebras response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<CerebrasStreamResponse>(data);
    ...
    private CerebrasRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new CerebrasMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        var request = new CerebrasRequest
        {
            Model = _modelId,
            Messages = messages,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new CerebrasTool
                {
                    Function = new CerebrasFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(CerebrasResponse response)
    {
    ...
    private static Usage MapUsage(CerebrasUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<CerebrasErrorResponse>(content);
                throw new CerebrasException(
                    errorResponse?.Error.Message ?? "Cerebras API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new CerebrasException($"Cerebras API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
