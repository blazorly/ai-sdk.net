...
using AiSdk.Providers.DeepSeek.Models;

namespace AiSdk.Providers.DeepSeek;

/// <summary>
/// DeepSeek implementation of ILanguageModel.
/// </summary>
public class DeepSeekChatLanguageModel : ILanguageModel
{
    ...
    {
        // DeepSeek currently does not support image URLs like OpenAI
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="DeepSeekChatLanguageModel"/> class.
    /// </summary>
    ...
    public DeepSeekChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<DeepSeekResponse>(cancellationToken)
            ?? throw new DeepSeekException("Failed to deserialize DeepSeek response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<DeepSeekStreamResponse>(data);
    ...
    private DeepSeekRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new DeepSeekMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new DeepSeekTool
                {
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(DeepSeekResponse response)
    {
    ...
    private static Usage MapUsage(DeepSeekUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<DeepSeekErrorResponse>(content);
                throw new DeepSeekException(
                    errorResponse?.Error.Message ?? "DeepSeek API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new DeepSeekException($"DeepSeek API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
