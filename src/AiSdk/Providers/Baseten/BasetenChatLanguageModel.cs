...
using AiSdk.Providers.Baseten.Models;

namespace AiSdk.Providers.Baseten;

/// <summary>
/// Baseten implementation of ILanguageModel.
/// </summary>
public class BasetenChatLanguageModel : ILanguageModel
{
    private readonly HttpClient _httpClient;
    private readonly BasetenConfiguration _config;
    ...
    /// </summary>
    public string Provider => "baseten";
    ...
    {
        // Baseten currently does not support image URLs
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BasetenChatLanguageModel"/> class.
    /// </summary>
    ...
    public BasetenChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<BasetenResponse>(cancellationToken)
            ?? throw new BasetenException("Failed to deserialize Baseten response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<BasetenStreamResponse>(data);
    ...
    private BasetenRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new BasetenMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        var request = new BasetenRequest
        {
            Model = _modelId,
            Messages = messages,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new BasetenTool
                {
                    Function = new BasetenFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(BasetenResponse response)
    {
    ...
    private static Usage MapUsage(BasetenUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<BasetenErrorResponse>(content);
                throw new BasetenException(
                    errorResponse?.Error.Message ?? "Baseten API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new BasetenException($"Baseten API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
