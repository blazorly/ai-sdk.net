...
using AiSdk.Providers.Perplexity.Models;

namespace AiSdk.Providers.Perplexity;

/// <summary>
/// Perplexity implementation of ILanguageModel.
/// Supports both standard instruction-following models and online search-enabled Sonar models.
/// </summary>
public class PerplexityChatLanguageModel : ILanguageModel
{
    ...
    /// </summary>
    public string Provider => "perplexity";
    ...
    {
        // Perplexity currently does not support image URLs in the same way as OpenAI
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="PerplexityChatLanguageModel"/> class.
    /// </summary>
    ...
    public PerplexityChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<PerplexityResponse>(cancellationToken)
            ?? throw new PerplexityException("Failed to deserialize Perplexity response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<PerplexityStreamResponse>(data);
    ...
    private PerplexityRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new PerplexityMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new PerplexityTool
                {
                    Function = new PerplexityFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(PerplexityResponse response)
    {
    ...
    private static Usage MapUsage(PerplexityUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<PerplexityErrorResponse>(content);
                throw new PerplexityException(
                    errorResponse?.Error.Message ?? "Perplexity API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new PerplexityException($"Perplexity API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
