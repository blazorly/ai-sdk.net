...
using AiSdk.Providers.Fal.Models;

namespace AiSdk.Providers.Fal;

/// <summary>
/// Fal AI implementation of ILanguageModel.
/// Fal AI provides fast inference for image generation, video generation, and LLMs.
/// </summary>
public class FalChatLanguageModel : ILanguageModel
{
    ...
    {
        // Fal AI primarily focuses on image/video generation
        // LLM support may have limited URL support
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="FalChatLanguageModel"/> class.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var response = await _httpClient.PostAsync("fal-ai/any-llm", content, cancellationToken);
        await EnsureSuccessStatusCode(response);

        var result = await response.Content.ReadFromJsonAsync<FalResponse>(cancellationToken)
            ?? throw new FalException("Failed to deserialize Fal AI response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var httpRequest = new HttpRequestMessage(HttpMethod.Post, "fal-ai/any-llm")
        {
            Content = content
        };
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<FalStreamResponse>(data);
    ...
    private void ConfigureHttpClient()
    {
        ...
        _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Key", _config.ApiKey);
    ...
    private FalRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new FalMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new FalTool
                {
                    Function = new FalFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(FalResponse response)
    {
    ...
    private static Usage MapUsage(FalUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<FalErrorResponse>(content);
                throw new FalException(
                    errorResponse?.Error.Message ?? "Fal AI API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new FalException($"Fal AI API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
