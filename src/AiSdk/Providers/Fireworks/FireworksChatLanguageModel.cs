...
using AiSdk.Providers.Fireworks.Models;

namespace AiSdk.Providers.Fireworks;

/// <summary>
/// Fireworks AI implementation of ILanguageModel.
/// Provides fast inference for open-source LLMs with OpenAI-compatible API.
/// </summary>
public class FireworksChatLanguageModel : ILanguageModel
{
    private readonly HttpClient _httpClient;
    private readonly FireworksConfiguration _config;
    ...
    /// </summary>
    public string Provider => "fireworks";
    ...
    {
        // Fireworks AI supports image URLs natively for vision models
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="FireworksChatLanguageModel"/> class.
    /// </summary>
    ...
    public FireworksChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<FireworksResponse>(cancellationToken)
            ?? throw new FireworksException("Failed to deserialize Fireworks AI response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<FireworksStreamResponse>(data);
    ...
    private FireworksRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new FireworksMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        var request = new FireworksRequest
        {
            Model = _modelId,
            Messages = messages,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new FireworksTool
                {
                    Function = new FireworksFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(FireworksResponse response)
    {
    ...
    private static Usage MapUsage(FireworksUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<FireworksErrorResponse>(content);
                throw new FireworksException(
                    errorResponse?.Error.Message ?? "Fireworks AI API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new FireworksException($"Fireworks AI API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
