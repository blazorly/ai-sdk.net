...
using AiSdk.Providers.Writer.Models;

namespace AiSdk.Providers.Writer;

/// <summary>
/// Writer implementation of ILanguageModel.
/// </summary>
public class WriterChatLanguageModel : ILanguageModel
{
    ...
    {
        // Writer currently does not support image URLs like OpenAI
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="WriterChatLanguageModel"/> class.
    /// </summary>
    ...
    public WriterChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<WriterResponse>(cancellationToken)
            ?? throw new WriterException("Failed to deserialize Writer response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<WriterStreamResponse>(data);
    ...
    private WriterRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new WriterMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new WriterTool
                {
                    Function = new WriterFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(WriterResponse response)
    {
    ...
    private static Usage MapUsage(WriterUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<WriterErrorResponse>(content);
                throw new WriterException(
                    errorResponse?.Error.Message ?? "Writer API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new WriterException($"Writer API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
