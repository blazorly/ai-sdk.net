...
using AiSdk.Providers.AI21.Models;

namespace AiSdk.Providers.AI21;

/// <summary>
/// AI21 Labs implementation of ILanguageModel.
/// Provides access to Jamba and Jurassic models with OpenAI-compatible API.
/// </summary>
public class AI21ChatLanguageModel : ILanguageModel
{
    private readonly HttpClient _httpClient;
    private readonly AI21Configuration _config;
    ...
    {
        // AI21 Labs supports image URLs natively for vision-capable models
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="AI21ChatLanguageModel"/> class.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<AI21Response>(cancellationToken)
            ?? throw new AI21Exception("Failed to deserialize AI21 Labs response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<AI21StreamResponse>(data);
    ...
    private AI21Request BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new AI21Message
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new AI21Tool
                {
                    Function = new AI21Function
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(AI21Response response)
    {
    ...
    private static Usage MapUsage(AI21Usage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<AI21ErrorResponse>(content);
                throw new AI21Exception(
                    errorResponse?.Error.Message ?? "AI21 Labs API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new AI21Exception($"AI21 Labs API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
