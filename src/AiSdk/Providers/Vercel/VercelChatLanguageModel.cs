...
using AiSdk.Providers.Vercel.Models;

namespace AiSdk.Providers.Vercel;

/// <summary>
/// Vercel AI Gateway implementation of ILanguageModel.
/// Provides access to models from multiple providers through Vercel's unified API gateway.
/// </summary>
public class VercelChatLanguageModel : ILanguageModel
{
    private readonly HttpClient _httpClient;
    private readonly VercelConfiguration _config;
    ...
    /// Format: "provider/model-name" (e.g., "openai/gpt-4", "anthropic/claude-opus-4.5").
    /// </summary>
    ...
    {
        // Vercel AI Gateway supports image URLs through compatible providers like OpenAI
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="VercelChatLanguageModel"/> class.
    /// </summary>
    ...
    public VercelChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<VercelResponse>(cancellationToken)
            ?? throw new VercelException("Failed to deserialize Vercel AI Gateway response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<VercelStreamResponse>(data);
    ...
    private VercelRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new VercelMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new VercelTool
                {
                    Function = new VercelFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(VercelResponse response)
    {
    ...
    private static Usage MapUsage(VercelUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<VercelErrorResponse>(content);
                throw new VercelException(
                    errorResponse?.Error.Message ?? "Vercel AI Gateway API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new VercelException($"Vercel AI Gateway API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
