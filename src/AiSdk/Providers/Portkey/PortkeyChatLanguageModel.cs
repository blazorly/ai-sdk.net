...
using AiSdk.Providers.Portkey.Models;

namespace AiSdk.Providers.Portkey;

/// <summary>
/// Portkey AI Gateway implementation of ILanguageModel.
/// Provides access to models from multiple providers through Portkey's unified API gateway.
/// </summary>
public class PortkeyChatLanguageModel : ILanguageModel
{
    private readonly HttpClient _httpClient;
    private readonly PortkeyConfiguration _config;
    ...
    /// </summary>
    public string Provider => "portkey";
    ...
    {
        // Portkey AI Gateway supports image URLs through compatible providers like OpenAI
        var supported = new Dictionary<string, IReadOnlyList<string>>
        {
            ["image/*"] = new List<string> { "https://*", "http://*", "data:*" }
        };
        return Task.FromResult<IReadOnlyDictionary<string, IReadOnlyList<string>>>(supported);
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="PortkeyChatLanguageModel"/> class.
    /// </summary>
    ...
    public PortkeyChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<PortkeyResponse>(cancellationToken)
            ?? throw new PortkeyException("Failed to deserialize Portkey AI Gateway response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<PortkeyStreamResponse>(data);
    ...
    private void ConfigureHttpClient()
    {
        ...
        _httpClient.DefaultRequestHeaders.Add("x-portkey-api-key", _config.ApiKey);

        if (!string.IsNullOrEmpty(_config.Provider))
        {
            _httpClient.DefaultRequestHeaders.Add("x-portkey-provider", _config.Provider);
        }

        if (!string.IsNullOrEmpty(_config.VirtualKey))
        {
            _httpClient.DefaultRequestHeaders.Add("x-portkey-virtual-key", _config.VirtualKey);
        }
    ...
    private PortkeyRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new PortkeyMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        var request = new PortkeyRequest
        {
            Model = _modelId,
            Messages = messages,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new PortkeyTool
                {
                    Function = new PortkeyFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(PortkeyResponse response)
    {
    ...
    private static Usage MapUsage(PortkeyUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<PortkeyErrorResponse>(content);
                throw new PortkeyException(
                    errorResponse?.Error.Message ?? "Portkey AI Gateway API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new PortkeyException($"Portkey AI Gateway API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
