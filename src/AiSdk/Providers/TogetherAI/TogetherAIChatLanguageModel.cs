...
using AiSdk.Providers.TogetherAI.Models;

namespace AiSdk.Providers.TogetherAI;

/// <summary>
/// Together AI implementation of ILanguageModel.
/// </summary>
public class TogetherAIChatLanguageModel : ILanguageModel
{
    ...
    /// </summary>
    public string Provider => "togetherai";
    ...
    {
        // Together AI supports image URLs in vision models
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="TogetherAIChatLanguageModel"/> class.
    /// </summary>
    ...
    public TogetherAIChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<TogetherAIResponse>(cancellationToken)
            ?? throw new TogetherAIException("Failed to deserialize Together AI response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<TogetherAIStreamResponse>(data);
    ...
    private TogetherAIRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new TogetherAIMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new TogetherAITool
                {
                    Function = new TogetherAIFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(TogetherAIResponse response)
    {
    ...
    private static Usage MapUsage(TogetherAIUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<TogetherAIErrorResponse>(content);
                throw new TogetherAIException(
                    errorResponse?.Error.Message ?? "Together AI API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new TogetherAIException($"Together AI API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
