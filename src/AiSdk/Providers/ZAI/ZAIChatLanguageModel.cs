...
using AiSdk.Providers.ZAI.Exceptions;
using AiSdk.Providers.ZAI.Models;

namespace AiSdk.Providers.ZAI;

/// <summary>
/// Z.AI implementation of ILanguageModel.
/// </summary>
public class ZAIChatLanguageModel : ILanguageModel
{
    ...
    {
        // Z.AI currently does not support image URLs like OpenAI
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ZAIChatLanguageModel"/> class.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<ZAIResponse>(cancellationToken)
            ?? throw new ZAIException("Failed to deserialize Z.AI response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<ZAIStreamResponse>(data);
    ...
    private ZAIRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new ZAIMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new ZAITool
                {
                    Function = new ZAIFunction
                    {
                        Name = t.Name,
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(ZAIResponse response)
    {
    ...
    private static Usage MapUsage(ZAIUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new ZAIException($"Z.AI API error: {content} (Status: {response.StatusCode})");
        }
    }
