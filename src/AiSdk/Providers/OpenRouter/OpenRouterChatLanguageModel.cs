...
using AiSdk.Providers.OpenRouter.Models;

namespace AiSdk.Providers.OpenRouter;

/// <summary>
/// OpenRouter implementation of ILanguageModel.
/// Provides access to 100+ models from multiple providers through OpenRouter's unified API.
/// </summary>
public class OpenRouterChatLanguageModel : ILanguageModel
{
    ...
    /// Format: "provider/model-name" (e.g., "openai/gpt-4-turbo", "anthropic/claude-3.5-sonnet").
    /// </summary>
    ...
    {
        // OpenRouter supports image URLs through compatible providers like OpenAI and Anthropic
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="OpenRouterChatLanguageModel"/> class.
    /// </summary>
    ...
    public OpenRouterChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<OpenRouterResponse>(cancellationToken)
            ?? throw new OpenRouterException("Failed to deserialize OpenRouter response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<OpenRouterStreamResponse>(data);
    ...
    private void ConfigureHttpClient()
    {
        ...
        {
            _httpClient.DefaultRequestHeaders.TryAddWithoutValidation("X-Title", _config.AppName);
        }
    ...
    private OpenRouterRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new OpenRouterMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new OpenRouterTool
                {
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(OpenRouterResponse response)
    {
    ...
    private static Usage MapUsage(OpenRouterUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<OpenRouterErrorResponse>(content);
            ...
            {
                throw new OpenRouterException($"OpenRouter API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
