...
using AiSdk.Providers.Groq.Models;

namespace AiSdk.Providers.Groq;

/// <summary>
/// Groq implementation of ILanguageModel.
/// </summary>
public class GroqChatLanguageModel : ILanguageModel
{
    ...
    {
        // Groq currently does not support image URLs like OpenAI
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="GroqChatLanguageModel"/> class.
    /// </summary>
    /// <param name="modelId">The Groq model ID (e.g., "llama-3.1-70b-versatile", "mixtral-8x7b-32768").</param>
    /// <param name="config">The Groq configuration.</param>
    /// <param name="httpClient">Optional HTTP client to use.</param>
    public GroqChatLanguageModel(
        string modelId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<GroqResponse>(cancellationToken)
            ?? throw new GroqException("Failed to deserialize Groq response");
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in ReadLinesAsync(stream, cancellationToken))
        {
            ...
            if (line.StartsWith("data: "))
            {
                ...
                var chunk = JsonSerializer.Deserialize<GroqStreamResponse>(data);
    ...
    private GroqRequest BuildRequest(LanguageModelCallOptions options, bool stream)
    {
        var messages = options.Messages.Select(m => new GroqMessage
        {
            Role = MapRole(m.Role),
            Content = m.Content,
        ...
        if (options.Tools?.Count > 0)
        {
            ...
            {
                Tools = options.Tools.Select(t => new GroqTool
                {
    ...
    private static LanguageModelGenerateResult MapToGenerateResult(GroqResponse response)
    {
    ...
    private static Usage MapUsage(GroqUsage usage) => new Usage(
    ...
    private static async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                var errorResponse = JsonSerializer.Deserialize<GroqErrorResponse>(content);
                throw new GroqException(
                    errorResponse?.Error.Message ?? "Groq API error",
                    (int)response.StatusCode,
                    errorResponse?.Error.Code);
            }
            catch (JsonException)
            {
                throw new GroqException($"Groq API error: {content}", (int)response.StatusCode, null);
            }
        }
    }
